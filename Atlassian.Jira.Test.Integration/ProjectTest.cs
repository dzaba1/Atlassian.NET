...
public class ProjectTest
{
    ...
    public async Task AddAndRemoveProjectComponent(Jira jira)
    {
        ...
        var project = (await jira.Projects.GetProjectsAsync()).First();
        ...
        Assert.Contains(await project.GetComponentsAsync(), p => p.Name == componentName);

        // Delete project component
        await project.DeleteComponentAsync(component.Name);
        Assert.DoesNotContain(await project.GetComponentsAsync(), p => p.Name == componentName);
    }
    ...
    {
        var components = await jira.Components.GetComponentsAsync("TST");
    ...
    public async Task GetAndUpdateProjectVersions(Jira jira)
    {
        ...
        var versions = await jira.Versions.GetVersionsAsync("TST");
        ...
        version = (await jira.Versions.GetVersionsAsync("TST")).First(v => v.Name == "1.0");
        Assert.Equal(newDescription, version.Description);
